...
/**
 * parser.java ??? Analisador Sint??tico Recursive Descent para Java--
 *
 * Gram??tica suportada (ver Parser.cup para especifica????o completa):
 *   programa, declara????es de vari??veis e fun????es, bloco, comandos,
 *   if/else, while, for, return, print, read, chamadas, express??es.
 *
 * Recupera????o de erros: panic mode com limite de avan??o para evitar loops.
 *
 * Linguagem: Java-- (Compiladores ??? 7?? CC)
 */
public class parser {
    ...
    /** Consome se for o tipo esperado; caso contr??rio emite erro mas N??O consome. */
    ...
    /** Panic mode: consome tokens at?? achar um ponto de sincroniza????o. */
    private void sincronizar(Set<Integer> sync) {
        ...
        // Consome ';' se for o ponto de parada, para avan??ar ao pr??ximo stmt
    ...
    private void parseVarDecl() {
        ...
        // M??ltiplos identificadores: int a, b, c;
    ...
    private void parseBloco() {
        ...
        while (!eh(sym.RBRACE) && !eh(sym.EOF)) {
            Token antes = atual();
            parseStmt();
            // Prote????o anti-loop: se nada foi consumido, for??a
    ...
    private void parseStmt() {
        ...
        try {
            switch (t.tipo) {
                ...
                // Tipos primitivos ??? declara????o local
    ...
    // Stmts que come??am com IDENT: decl de classe, atrib, chamada
    private void parseIdentStmt() {
        // IDENT IDENT ... ??? declara????o com tipo de usu??rio
        if (prox().tipo == sym.IDENT) {
            parseVarDecl();
            return;
        }
        // IDENT = expr ;  ???  atribui????o
        ...
        // IDENT . ... ;  ???  chamada de m??todo ou acesso a campo
    ...
    private void parsePrimary() {
        ...
        switch (t.tipo) {
            ...
            default:
                ...
                // N??o consome ??? deixa o caller lidar
        }
    }
