...
%{
  ...
  // writer compartilhado: escreve em arquivo + console simultaneamente
  static java.io.PrintWriter out = null;

  private void printToken(String tipo, String valor) {
    String linha = "[" + (yyline + 1) + "," + (yycolumn + 1) + "] " + tipo + ": " + valor;
    System.out.println(linha);
    if (out != null) out.println(linha);
  }

  private void printError(String mensagem) {
    String linha = "[" + (yyline + 1) + "," + (yycolumn + 1) + "] ERRO_LEXICO: " + mensagem;
    System.out.println(linha);
    if (out != null) out.println(linha);
  }
%}
...
<YYINITIAL> {
  ...
  "boolean"               { printToken("KEYWORD", yytext()); }
  "char"                  { printToken("KEYWORD", yytext()); }
  "String"                { printToken("KEYWORD", yytext()); }
  "for"                   { printToken("KEYWORD", yytext()); }
  "true"                  { printToken("KEYWORD", yytext()); }
  "false"                 { printToken("KEYWORD", yytext()); }
  "null"                  { printToken("KEYWORD", yytext()); }
...
<COMMENT> {
  ...
  <<EOF>>                 {
                            String msgEof = "[" + commentStartLine + "," + commentStartColumn + "] ERRO_LEXICO: comentario de multiplas linhas nao fechado";
                            System.out.println(msgEof);
                            if (out != null) out.println(msgEof);
                            return YYEOF;
                          }
}
